#!/bin/bash	  
#SBATCH -J CCO_energy	 	# Job Name
#SBATCH -o CCO_energy.o%j	 # Name of the error and output files (%j appends the jobID)
...
deform_list="60 70 80 90 100 110 120 130 140 150 160"
for ener in $deform_list
do
cat > cco.$ener.ener_new.in << EOF
&CONTROL
...
mpirun -np 300 pw.x < cco.$ener.ener_new.in > cco.$ener.ener_new.out # Run the executable named pw.x 
